...
def register_transaction_callbacks(app):
    ...
    def handle_add_transaction(
        n_clicks, trans_type, amount, description, date, category_id, account_id
    ):
        ...
        if n_clicks > 0:
            ...
            if success:
                return _create_success_message(
                    f"✓ {trans_type.capitalize()} added successfully!"
                )
            else:
                return _create_error_message(f"✗ {message}")

    @app.callback(
        Output("transaction-category", "options"), Input("transaction-type", "value")
    )
